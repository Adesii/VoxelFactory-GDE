...
void VoxelWorld::init() {
	...
	for(int x= -view_distance; x <= view_distance; ++x){
		for(int z=-view_distance; z<=view_distance;++z){
			...
			add_child(chunk);
			chunk->set_chunk_position(Vector3i(x,0,z));
			chunk->set_name("Chunk_" + Variant(x).stringify() + "_" + Variant(z).stringify());
			add_chunk(chunk);
			godot::UtilityFunctions::print("Added chunk at ",chunk->get_chunk_position(),"\n");
		}
	}
...
void VoxelWorld::single_thread_generate(){
	if (!generating && timesincence > 1.0f) {
    ...
    {
        meshResult* c = memnew(meshResult(Array(),Vector3i(0,0,0)));
