...
public:
	...
	inline Span(Span<T> &p_other, size_t p_begin, size_t p_end) {
		assert(p_end >= p_begin);
		assert(p_begin < p_other.size());
		assert(p_end <= p_other.size()); // `<=` because p_end is typically `p_begin + size`
...
#ifdef DEBUG_ENABLED
		assert(size_in_bytes % sizeof(U) == 0);
#endif
